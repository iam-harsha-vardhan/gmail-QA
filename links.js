(async function () {
  ...
  return (
    ...
    // width="1" height="1"
    (
      /\bwidth\s*=\s*["']?(?:0|1)(?:px)?["']?/i.test(tag) &&
      /\bheight\s*=\s*["']?(?:0|1)(?:px)?["']?/i.test(tag)
    ) ||

    // style="width:1px;height:1px"
    (
      /width\s*:\s*(?:0|1)px/i.test(tag) &&
      /height\s*:\s*(?:0|1)px/i.test(tag)
    ) ||
    ...
    /opacity\s*:\s*0/i.test(tag) ||

    /max-width\s*:\s*(?:0|1)px/i.test(tag) ||

    /max-height\s*:\s*(?:0|1)px/i.test(tag)
